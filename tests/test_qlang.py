...
def test_col_eval():
    ...
    text = 'failed test0: copy col contents\nDIFF:\n' + qp.diff(result, expected).str()
    ...
    expected['ID'] = expected['name']
    text = (
        'failed test1: copy col contents and select all\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    text = (
        'failed test2: copy col contents to multiple cols\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    text = (
        'failed test3: copy col contents to all cols\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['age'] = expected['name']
    text = (
        'failed test4: conditionally copy col contents to multiple cols\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['age'] = expected['name']
    text = (
        'failed test5: conditionally copy col contents to multiple cols\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['ID'] = expected['name']
    rows = [0, 1, 2, 3, 4, 8, 10]
    expected.loc[rows, 'age'] = expected.loc[rows, 'name']
    text = (
        'failed test6: conditionally copy col contents to multiple cols\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert (result.loc[[5, 6, 7, 9], 'age'] != df.loc[[5, 6, 7, 9], 'name']).all(), text
    text = (
        'failed test7: conditionally copy col contents to multiple cols\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
...
def test_eval():
    ...
    text = (
        'failed test0: change to lowercase\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['name'] = expected['name'].str.lower()
    text = (
        'failed test1: conditionally change to lowercase\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    text = (
        'failed test2: convert and change to lowercase\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    text = (
        'failed test3: convert single entry to str\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['age'] = str(0)
    text = (
        'failed test4: conditionally convert multiple entries to str\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    text = (
        'failed test5: conditionally convert multiple entries to str\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    text = (
        'failed test6: conditionally convert multiple entries to str\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert (result['ID'] == 10).all(), text
    text = (
        'failed test7: conditionally convert multiple entries to str\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert (result.loc[[5, 6, 7, 9], 'age'] != 10).all(), text
    text = (
        'failed test8: conditionally convert multiple entries to str\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['name'] = '1'
    text = (
        'failed test9: convert all entries to python expression\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
...
params = [
    (
        r'id  $cols id',
        get_df().rename(columns={'ID': 'id'}).loc[:, ['id']]
    ),
    (
        r'id  $cols id   %name  $cols n   %date of birth  $cols dob  %is any;',
        get_df().rename(columns={'ID': 'id', 'name': 'n', 'date of birth': 'dob'})
    ),
    (
        r'id  $cols += abc',
        get_df().rename(columns={'ID': 'IDabc'}).loc[:, ['IDabc']]
    ),
    (
        r'id / name / date of birth   $cols += abc  %is any;',
        get_df().rename(
            columns={
                'ID': 'IDabc',
                'name': 'nameabc',
                'date of birth': 'date of birthabc',
                }
            )
    ),
    (
        r'id  $cols ~ x.lower() + str(len(x))   %is any;',
        get_df().rename(columns={'ID': 'id2'})
    ),
    (
        r'id / weight / diabetes    $cols ~ x.lower() + str(len(x))   %is any;',
        get_df().rename(
            columns={
                'ID': 'id2',
                'weight': 'weight6',
...
def test_cols(code, expected):
    ...
    text = (
        'failed test5: rename multiple col cols with eval\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
...
def test_logging():
    ...
    assert result.equals(expected), qp.diff(result, expected).str()
    assert result1.equals(expected), qp.diff(result1, expected).str()
    check_message(
        'WARNING: value 0 will be ignored for unary operator <"is na;": IS_NA>'
        )
    ...
    assert result.equals(expected), qp.diff(result, expected).str()
    check_message(
        'ERROR: col "test" not found in dataframe.'
        ' cannot use "@test" for row selection'
        )
    ...
    check_message(
        'WARNING: verbosity must be an integer between 0 and 5. "6" is not valid'
        )
    ...
    check_message(
        'WARNING: diff must be one of [None, mix, old, new, new+]. "old+" is not valid'
        )

    result1 = df.q(r'$diff=None')
    result2 = df.q(r'$diff=0')
    result3 = df.q(r'$diff=false')
    text = f'setting diff to None should return a dataframe, not {type(result1)}'
    assert isinstance(result1, pd.DataFrame), text
    assert result1.equals(result2), qp.diff(result1, result2).str()
    assert result1.equals(result3), qp.diff(result1, result3).str()

    log(clear=True, verbosity=1)
    result = df.q(r'test  $cols=abc')
    check_message(
        'ERROR: colname modification cannot be applied when no cols where selected'
        )
...
params = [
    ...
    (
        r'=a   %%>0            $meta ~ x + ">0"            %is any;  %%is any;',
        ['', '', '>0']
    ),
    (
        r'=a   %%>0            $meta ~ ">" + str(0)        %is any;  %%is any;',
        ['', '', '>0']
    ),
    (
        r'=a   %%>0            $meta col~ col + ">0"       %is any;  %%is any;',
        ['', '', '>0']
    ),
    (
...
def test_metadata(code, metadata):
    ...
    text = 'failed test: metadata tagging\nDIFF:\n' + qp.diff(result, expected).str()
    assert result.equals(expected), text
...
def test_metadata_init():
    ...
    expected = df.copy()
    expected['meta'] = ['a', 'a', 'a']
    text = 'failed test: metadata init\nDIFF:\n' + qp.diff(result, expected).str()
    assert result.equals(expected), text
...
def test_metadata_continous():
    ...
    expected['meta'] = ['', '', '>0']
    text = (
        'failed test0: continous metadata tagging\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['meta'] = ['', '', '>0>0']
    text = (
        'failed test1: continous metadata tagging\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['meta'] = ['', '0', '>0>0']
    text = (
        'failed test2: continous metadata tagging\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['meta'] = ['', '', '>0>0']
    text = (
        'failed test3: continous metadata tagging\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['meta'] = ['', '', '>>']
    text = (
        'failed test4: continous metadata tagging\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected['meta'] = ['', '', '']
    text = (
        'failed test5: continous metadata tagging\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
...
params = [
    ...
    (
        r'%age                             $cols bg=orange',
        qp.get_df().index
    ),
    (
        r'%age                             $rows bg=orange',
        qp.get_df().index
    ),
    (
        r'%age                             $vals bg=orange',
        qp.get_df().index
    ),
    (
        r'%age     %%>20                   $bg=orange          %%is any;',
        [1, 4, 10]
    ),
    (
        r'%age     %%>20                   $cols bg=orange     %%is any;',
        [1, 4, 10]
    ),
    (
        r'%age     %%>20                   $rows bg=orange     %%is any;',
        [1, 4, 10]
    ),
    (
        r'%age     %%>20                   $vals bg=orange     %%is any;',
        [1, 4, 10]
    ),
    (
        r'%age     %%>20       %%%>30      $bg=orange          %%is any;',
        [4, 10]
    ),
    (
        r'%age     %%>20       %%%>30      $vals bg=orange     %%is any;',
        [4, 10]
    ),
    (
        r'%age     %%>20       %%%>30      $rows bg =orange    %%is any;',
        [1, 4, 10]
    ),
    (
        r'%age     %%>20       %%%>30      $cols bg = orange   %%is any;',
        [1, 4, 10]
    ),
    ]
...
def test_new_col1():
    ...
    text = (
        'failed test1: creating new col with colname\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
...
params = [
    ...
    (
        r'date of birth  $rows to datetime;        %%=05-11-2007',
        df.loc[[4], ['date of birth']],
        None
    ),
    (
        r'date of birth  $vals to datetime;        %%>1990-01-01',
        df.loc[[0, 1, 4], ['date of birth']],
        None
    ),
    (
        r'date of birth  %%is any;  $to datetime;  %%>1990-01-01  &&<2000-01-01',
        df.loc[[0, 1], ['date of birth']],
        None
    ),
    ...
    (
        r'weight  %%strict is num;',
        df.loc[[0, 10], ['weight']],
        None
    ),
    (
        r'weight  %%is num;  &&!is na;',
        df.loc[[0, 1, 7, 9, 10], ['weight']],
        None
    ),
    ...
    (
        r'diabetes     %%strict is bool;',
        df.loc[[], ['diabetes']],
        None
    ),

    (
        r'date of birth  %%is date;',
        df.loc[:, ['date of birth']],
        None
    ),
    (
        r'date of birth  %%is datetime;',
        df.loc[:, ['date of birth']],
        None
    ),
    ...
    (
        ...
        r'name %%regex=\b[A-Z][a-z]*\s[A-Z][a-z]*\b',
    ...
    #using regex search
    (
        r'bp systole %%regex?m',
        df.loc[[4], ['bp systole']],
        None
    ),
    (
        r'bp systole %%regex?\D',
        df.loc[[2, 4, 6], ['bp systole']],
        None
    ),
    (
        r'bp systole %%regex?\d',
        df.loc[[0, 1, 3, 4, 5, 7, 9, 10], ['bp systole']],
        None
    ),
    ...
    (
        r'age / height  $rows to num;   %height  %%>@age',
        df.loc[[0, 10], ['height']],
        None
    ),
    (
        r'age / height  $vals to num;   %height  %%<@age',
        df.loc[[], ['height']],
        None
    ),
    (
        r'cholesterol   %%=@bp systole',
        df.loc[[2], ['cholesterol']],
        None
    ),
    ...
    (
        r'id / name  %%?j  //?n',
        df.loc[[0, 1, 2, 3, 5, 8, 9, 10], ['ID', 'name']],
        None
    ),
    (
        r'id / name  %%?j  &&?n',
    ...
    (
        r'height / weight   %%is num;',
        df.loc[:, ['height', 'weight']],
        None
    ),
    (
        r'height / weight   %%any is num;',
        df.loc[:, ['height', 'weight']],
        None
    ),
    (
        r'height / weight   %%anyis num;',
        df.loc[:, ['height', 'weight']],
        None
    ),
    (
        r'height / weight   %%all is num;',
...
def test_select_rows_scopes():
    ...
    expected.loc[:, :] = ''
    text = (
        'failed test0: check if default selection scope is "any"\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected.loc[:, :] = ''
    text = (
        'failed test1: check selection scope "any"\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    ...
    expected = get_df().loc[[], :]
    text = (
        'failed test2: check selection scope "all"\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
    check_message(
        'WARNING: modification cannot be applied when no values where selected'
        )
...
def test_select_vals():
    ...
    text = (
        'failed test0: check val selection\nDIFF:\n'
        + qp.diff(result, expected).str()
        )
    assert result.equals(expected), text
...
def test_symbols():
    ...
    assert qp.qlang.OPERATORS['x'] is None, 'should return None for unknown symbol'
...
def test_type_inference():
    ...
    expected1 = df1.loc[:, [1]]
    expected2 = df2.loc[:, ['1']]
    assert result1.equals(expected1), qp.diff(result1, expected1).str()
    assert result2.equals(expected2), qp.diff(result2, expected2).str()
