...
def test_mode_new(tmpdir):
    ...
    result = qp.diff(df_new, df_old, 'new', uid='uid', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_new_ignore(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'new', uid='uid', ignore='a', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_new_ignore1(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'new', uid='uid', ignore=['b'], verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_new_ignore2(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'new', uid='uid', ignore=['a', 'b'], verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_newplus_ignore2(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'new+', uid='uid', ignore=['b', 'a'], verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new+".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_old(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'old', uid='uid', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "old".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_old_ignore(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'old', uid='uid', ignore='a', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "old".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_old_ignore1(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'old', uid='uid', ignore='b', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "old".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_old_ignore2(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'old', uid='uid', ignore=['a', 'b'], verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "old".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_mix(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'mix', uid='uid', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "mix".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_mix_ignore(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'mix', uid='uid', ignore='a', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "mix".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_mix_ignore1(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'mix', uid='uid', ignore=['b'], verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "mix".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_mode_mix_ignore2(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'mix', uid='uid', ignore=['a','b'], verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "mix".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_returns_df(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'new', uid='uid', output='df', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_returns_df_ignore(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'new', uid='uid', ignore='a', output='df', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_returns_df_ignore1(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'new', uid='uid', ignore=['b'], output='df', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_returns_df_ignore2(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, 'new', uid='uid', ignore=['b', 'a'], output='df', verbosity=0)['Sheet1'].data.astype('object')
    assert result.equals(expected), f'failed test for mode: "new".\nEXPECTED:\n{expected}\nRESULT:\n{result}'
...
def test_returns_summary(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, uid='uid', output='summary')
...
def test_returns_summary_ignore(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, uid='uid', ignore='a', output='summary')
    expected = pd.DataFrame({
        'sheet': ['Sheet1'],
        'cols added': [1],
        'cols removed': [1],
...
def test_returns_summary_ignore1(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, uid='uid', ignore=['b'], output='summary')
    expected = pd.DataFrame({
        'sheet': ['Sheet1'],
        'cols added': [1],
        'cols removed': [1],
...
def test_returns_summary_ignore2(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, uid='uid', ignore=['a','b'], output='summary')
    expected = pd.DataFrame({
        'sheet': ['Sheet1'],
        'cols added': [1],
        'cols removed': [1],
...
def test_returns_all(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result_dfs, result_summary, result_str = qp.diff(df_new, df_old, uid='uid', output='all')
...
def test_returns_print(tmpdir):
    ...
    #reading from files
    df_new, df_old = setup(df_new, df_old, tmpdir)
    result = qp.diff(df_new, df_old, uid='uid', output='print')
    assert result == expected, f'\nRESULT\n{result}\nEXPECTED:\n{expected}'
