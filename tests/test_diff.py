...
def test_mode_new_ignore2(tmpdir):
    ...
    result = qp.diff(
        df_new, df_old, 'new', ignore=['a', 'b'], verbosity=0).data
...
def test_returns_str(tmpdir):
    ...
    assert result == 'no string version of differences available when comparing excel files'  # noqa E501
...
def test_returns_all(tmpdir):
    ...
    result_df_styler, result_summary, result_str = qp.diff(
        df_new,
        df_old,
        uid='uid',
        output='all',
    ...
    assert result_str == 'no string version of differences available when comparing excel files'  # noqa E501
...
def test_identical(tmpdir):
    ...
    df1, summary1, str1 = qp.diff(
        df_new, df_new, uid='uid', mode='mix', output='all')
    dict2, summary2, str2 = qp.diff(
        df_new_xlsx, df_new_xlsx, uid='uid', mode='mix', output='all')
    df3, summary3, str3 = qp.diff(
        df_new_csv, df_new_csv, uid='uid', mode='mix', output='all')
    ...
    assert str2 == 'no string version of differences available when comparing excel files'  # noqa E501
